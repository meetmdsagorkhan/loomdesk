...
export default function TimeTrackingPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case "full": return "bg-emerald-500/20 text-emerald-400 border-emerald-500/50";
      case "ahead": return "bg-blue-500/20 text-blue-400 border-blue-500/50";
      case "behind": return "bg-red-500/20 text-red-400 border-red-500/50";
  ...
  return (
    <div className="p-6 md:p-10 space-y-8 max-w-7xl mx-auto">
      <div className="flex flex-col gap-2">
        <h1 className="text-4xl font-bold tracking-tight text-white">Time Tracking</h1>
        <p className="text-gray-400">View your work hours, check-ins, and schedule adherence.</p>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="bg-black/40 border-white/10 backdrop-blur-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-gray-400">Total Time This Month</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">Based on completed check-outs</p>
          </CardContent>
        </Card>

        <Card className="bg-black/40 border-white/10 backdrop-blur-xl">
          <CardHeader className="flex flex-row items-center justify-between pb-2">
            <CardTitle className="text-sm font-medium text-gray-400">Average Daily Hours</CardTitle>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">Over {entries.length} tracked days</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="bg-black/40 border-white/10 backdrop-blur-xl">
        <CardHeader>
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-gray-400">Your adherence to assigned shifts.</CardDescription>
            </div>
        ...
        <CardContent>
          <div className="flex items-center gap-6 mb-6 text-sm text-gray-400 bg-white/5 p-3 rounded-lg border border-white/5">
          ...
          <div className="grid grid-cols-7 gap-2">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((d) => (
              <div key={d} className="text-center text-gray-500 text-xs font-semibold py-2">
                {d}
              </div>
            ))}
            ...
            {Array.from({ length: startOfMonth(currentMonth).getDay() }).map((_, i) => (
              <div key={`empty-${i}`} className="h-24 rounded-xl bg-white/[0.02]" />
            ))}
        ...
        <CardContent className="space-y-4">
          {entries.filter(e => e.notes && e.notes.length > 0).slice(0, 5).map(entry => (
            <div key={entry.id} className="p-4 bg-white/5 rounded-xl border border-white/10">
              <div className="text-sm text-gray-400 mb-2">{format(new Date(entry.date), "MMMM d, yyyy")} - {Math.floor(entry.totalMinutes / 60)}h {entry.totalMinutes % 60}m</div>
              <div className="text-white text-sm">
                {entry.notes[0]?.content}
              </div>
            </div>
          ))}
          {entries.length === 0 && <div className="text-gray-500 text-sm">No notes available for this month.</div>}
        </CardContent>
      </Card>
    </div>
  );
