...
import { Loader2, Check, CheckCheck, X, Reply, Forward, Hash, Bell, Smile } from 'lucide-react';
...
export default function ChatWindow({ contactId, contactName, setSelectedContact }: ChatWindowProps) {
  ...
  const handleProfileClick = async (userId: string, userName: string) => {
    ...
    try {
      ...
      // Keep the basic profile info if fetch fails
    } finally {
            ...
            return (
              <div key={msg.id} className={`flex gap-2 ${isMe ? 'flex-row-reverse' : 'flex-row'}`}>
                ...
                >
                    ...
                    >
                      <div className="flex gap-1 overflow-x-auto">
                        {['😀', '😂', '😍', '👍', '🎉', '🔥', '❤️', '👋', '🙏', '✅', '😢', '😮', '😡', '🤔', '👎'].map((emoji) => (
                          <button
                            key={emoji}
                            onClick={(e) => { e.stopPropagation(); setShowEmojiPicker(false); setEmojiPickerMessageId(null); }}
                            className="w-10 h-10 text-2xl hover:bg-slate-100 dark:hover:bg-slate-700 rounded-lg transition-colors flex-shrink-0"
                          >
                            {emoji}
                          </button>
                        ))}
                  ...
                  {/* Reply/Forward/Emoji buttons on hover */}
                  <div className={`absolute top-1/2 -translate-y-1/2 ${isMe ? '-left-24' : '-right-24'} opacity-0 group-hover:opacity-100 transition-opacity flex gap-1 z-10`}>
                    <button
